...
function conferm_order(elem) {
    ...
    var id = elem.id;
    $.ajax({
        url: base_url + 'invoice/invoice/conferm_order_ajax',
        type: 'post',
        data: { id: id },
        success: function(msg) {
            toastr["success"]("Your order is Comformed!");
            chech_process();
        },
        error: function(xhr, desc, err) {
            alert('failed');
...
function update_order(elem) {
    var base_url = $("#base_url").val();
    var invoce_no = $("#" + elem.id).data("id");
    var id = elem.id;
    $.ajax({
        url: base_url + 'invoice/invoice/update_order_ajax',
        type: 'post',
        data: { id: id },
        success: function(msg) {
